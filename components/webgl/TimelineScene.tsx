...
const DayMarker: React.FC<DayMarkerProps> = ({ position, label }) => {
    ...
    // --- CORREÇÃO DE CRASH (Pacote 17) ---
    // A variável 'y' deve ser declarada ANTES de ser usada.
    const y = -2; // Posição Y da linha
    // --- FIM DA CORREÇÃO ---

    const textPosition = new Vector3(position.x, y - 0.5, position.z); // y - 0.5 = -2.5
    const lineStart = new Vector3(position.x, y, position.z); // y = -2
    const lineEnd = new Vector3(position.x, y + 1.5, position.z);   // y + 1.5 = -0.5
...
const TaskConnector: React.FC<{ start: Vector3, end: Vector3, color: string }> = ({ start, end, color }) => {
    ...
    return (
        <>
            ...
            <OrbitControls 
                ...
                mouseButtons={{
                    // @ts-ignore
                    MIDDLE: MOUSE.ROTATE, // Scroll-click = Orbitar
                    RIGHT: MOUSE.PAN      // Botão Direito = Arrastar
                }}
            />
